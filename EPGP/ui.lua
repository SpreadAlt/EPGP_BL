...
local function CreateEPGPFrame()
    ...
    t:SetTexture("Interface\\AddOns\\EPGP\\images\\blacklist")
    t:SetWidth(60)
    t:SetHeight(60)
...
local function CreateEPGPFrameStandings()
    ...
    function statusText:TextUpdate()
        ...
        local ep = name and EPGP:GetEPGP(name) or 0
        self:SetFormattedText("Ваше ЕП: |cFFFFFFFF%d|r", ep or 0)
    end

    EPGP.RegisterCallback(statusText, "StandingsChanged", "TextUpdate")

    -- Make the mode text
    local modeText = main:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    modeText:SetHeight(fontHeight)
    modeText:SetJustifyH("CENTER")
    modeText:SetPoint("BOTTOMLEFT", award, "TOPLEFT", 0, fontHeight)
    modeText:SetPoint("BOTTOMRIGHT", log, "TOPRIGHT", 0, fontHeight)
